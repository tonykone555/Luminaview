...
async function syncYnotAdmin(){if(YNOTAdmin.loading)return;YNOTAdmin.loading=true;renderApp();try{const r=await fetch(`${SUPABASE_URL}/functions/v1/ynot-feed?refresh=1`);const data=await r.json();if(!r.ok||data.error)throw new Error(data.error||'YNOT refresh failed');toast(`${data.items?.length||0} live YNOT products refreshed.`,'good');YNOTAdmin.loading=false;await loadYnotAdmin(false);renderApp()}catch(e){YNOTAdmin.loading=false;toast(e.message||'YNOT refresh failed','bad');renderApp()}}
